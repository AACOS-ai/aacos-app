  ...
  try {
    ...
    if (Number(recent[0].count) > 0) {
      return NextResponse.json(
        { error: "Rate limit: Wait 30s" },
        { status: 429 }
      );
    }

    // בדיקת כמות שידורים לשעה
    ...
    // שמירה אוטומטית – מודולרי: גם LOG גם אותות
    ...
    // אפשר להרחיב: שליחת LOG/QDRANT/AI notification כאן
...
export async function GET() {
  try {
    // אפשר להוסיף query params לקבלת פילטר/חיתוך (בעתיד)
